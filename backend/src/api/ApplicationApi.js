  ...
  async registerHandler() {
    try {
      ...
      this.router.post("/apply",
        ...
        async (req, res, next) => {
          try {
            ...
            logger.info(`Application submitted by user: ${req.user.id}`);
          } catch (err) {
      ...
      */
      this.router.patch("/update_application",
        ...
        async (req, res, next) => {
          try {
            ...
            logger.info(`Attempt to update application: ${req.body.job_application_id}`);

            if (!status) {
              this.sendResponse(res, 404, "status_not_found");
              logger.warn(`No status was found for application: ${req.body.job_application_id}`);
              return;
            }
            this.sendResponse(res, 200, status)
            logger.info(`Status of application: ${req.body.job_application_id}, was changed to ${status}`);
          } catch (err) {
            next(err);
          }
        });
      ...
      logger.error(`An error occured when trying to update application status: ${err.message}`);
      throw err;
    }
  }
}
